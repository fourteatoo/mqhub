...
```clojure
{:type :geo
 :areas {"home" {:lat 35.6594877446577 :lon 139.7005345136067
                 :radius 200            ; in meters
                 :enter (evo/set-location-mode ["Home"] :day-off)
                 :leave (evo/set-location-mode ["Home"] :auto)}
         "town" {:lat 35.73344853713392 :lon 139.70758047177927
                 :radius 5000           ; in meters
                 :enter (evo/set-location-mode ["Home"] :auto)
                 :leave (evo/set-location-mode ["Home"] :away)}}}
```

The data is supposed to be in JSON format and contain a map with at
least a type `location`, a `lat` and a `lon`; which is usually what
Owntracks sends.  The area transitions are determined by MQHUB, not
Owntracks.  You can as well specify no waypoint in the app.

You may have noticed that area overlaps may cause race conditions.
Here is an example as of how to avoid it:
...
{:type :geo
 :areas {"home" {:lat 35.6594877446577 :lon 139.7005345136067
                 :radius 200            ; in meters
                 :enter (evo/set-location-mode ["Home"] :day-off)
                 :leave (when (geo/present-in? "town" ctx)
                          (evo/set-location-mode ["Home"] :auto))}
         "town" {:lat 35.73344853713392 :lon 139.70758047177927
                 :radius 5000           ; in meters
                 :enter (when (not (geo/present-in? "home" ctx))
                          (evo/set-location-mode ["Home"] :auto))
                 :leave (evo/set-location-mode ["Home"] :away)}}}
```
...
The `:exec` subscription just invokes your Clojure code.  The `:exec`
